...
class DiscordPostViaWebhookWithInput(DiscordPostViaWebhook):
    """
    Variante du node avec entrée connectable pour le message (prompt).
    Basée sur DiscordPostViaWebhook pour ne rien casser.
    """
    @classmethod
    def INPUT_TYPES(cls):
        return {
            ...
            "optional": {
                "send_Message": ("BOOLEAN", {"default": False}),
                "send_Image": ("BOOLEAN", {"default": True}),
                # 👇 Ajout important: on force une ENTRÉE (socket) pour le texte
                "message": ("STRING", {"default": "", "multiline": True, "forceInput": True}),
                # Optionnel: on peut aussi forcer une entrée pour le prepend
                "prepend_message": ("STRING", {"default": "", "multiline": True, "forceInput": True}),
            },
        }

# Ajoute ces lignes à la fin des mappings existants
